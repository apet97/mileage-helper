...
- When the addon's high-precision `settings.rate()` differs from the Clockify unit-priced category's integer-cent price, the canonical line carries a `(Clockify category charge: <total>)` parenthetical that equals the real Clockify `total`, e.g. `… = 89.915252 (Clockify category charge: 89.90). Created/converted by …`. Identical amounts stay clean (no parenthetical). The **native** path here reads `total` from the `getExpense` snapshot, so this parenthetical is reliable. If you ALSO smoke the **add-on-create** path (`POST /api/mileage/expenses`), assert the same parenthetical on the created expense: `MileageApiController.resolveClockifyCategoryCharge` falls back to a `getExpense` snapshot because the addon-API create response omits `total` (live-QA 2026-06-05). Do NOT accept a missing parenthetical on a divergent add-on create — that is the production-only regression this fallback fixes.
