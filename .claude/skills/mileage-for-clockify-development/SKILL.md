...
| New Flyway migrations must be numbered AFTER the highest migration already present in the repo and after the highest applied production migration. The repo now contains pending `V21__index_mileage_note_reconcile_pending.sql`; hosted evidence still proves production only through V20 until deploy. | Flyway validates strict ordering and crashes boot with `Detected resolved migration not applied to database: N`. The d11e2088 deploy crashed on this with V7. The next new migration after this branch must be V22 or higher. |
