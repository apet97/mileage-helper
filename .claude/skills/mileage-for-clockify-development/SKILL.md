...
- An **add-on-form create** can now also fire one extra `EXPENSE_UPDATED` (loop-guard `SKIPPED`) of its own: after the create, `MileageApiController` reissues a single `updateFlatExpense` to stamp the `(Clockify category charge: X)` annotation when the Clockify create-response `total` differs from the rate amount. So during an add-on create smoke, expect up to `+1` worker timer / `+1 SKIPPED` beyond the create itself. If `total` matches (e.g. settings-save already synced the category price to the rate), no extra update fires.
