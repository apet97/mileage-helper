...
| New Flyway migrations must be numbered AFTER the highest migration already present in the repo and after the highest applied production migration. The repo now contains pending product migrations through `V24__add_mileage_trip_evidence.sql`; hosted evidence still proves production only through V20 until deploy. | Flyway validates strict ordering and crashes boot with `Detected resolved migration not applied to database: N`. The d11e2088 deploy crashed on this with V7. The next new migration after this branch must be V25 or higher. |
...
# /iframe/reimbursement-packet is wired + guarded: no auth_token => 401 (a real packet needs the iframe's auth_token query param)
...
After static asset deploys, live-click Mine, Team, and Conversions CSV buttons in the installed iframe and verify non-empty downloads plus `/api/mileage/*.csv` 200 metrics. CSV export clicks intentionally share the delegated `handleCsvExport` handler in `settings-tables.js`, wired once by boot `settings.js`; do not split them back into per-button listeners without live Conversions CSV proof. Also probe the report assets (`/assets/mileage/report.css`, `/assets/mileage/report.js`), packet assets (`/assets/mileage/packet.css`, `/assets/mileage/packet.js`), `GET /iframe/report` with no `auth_token` = `401`, and `GET /iframe/reimbursement-packet` with no `auth_token` = `401` (routes wired + guarded). The Report buttons (Mine + Team) open `/iframe/report` in a new tab via `handleReportClick`; Packet buttons open `/iframe/reimbursement-packet` via the same range/filter plumbing. The report lists ALL Clockify expenses (mileage rows show the add-on's reconciled miles/rate/amount; everything else native), can show/filter by live expense currency, and keeps blank-currency add-on audit rows visible. The Team report opens with NO user selected (admin → all users, adds a User column); selecting a user scopes it. Single-user report labels are resolved server-side from Clockify users, so the frontend must not pass display-name query parameters. Mine is always the requester's own. Native expenses come from `ClockifyExpenseGateway.listExpensesForReport` (backend `getExpenses`, paged, client-side date filter, `total` cents → major); it degrades to reconciled-mileage-only rows with a banner if Clockify is unreachable. After a deploy, open the report with no user selected and confirm both a mileage row (reconciled amount) and a non-mileage row (native amount) appear.

Product notes for this feature set: fresh workspaces default the rate to `0.725` (`MileageSettingsService.defaults()`); settings rate validation uses the same decimal bounds as create/preview; saving Settings may return `warnings` when the best-effort Clockify category price sync fails; admin rate policies resolve by expense date and audit `rateSource`/policy identity; optional trip evidence is audit-only and never appended to Clockify notes; the converted-note template is admin-editable in Settings (`settings-note-template`, ≤500 chars) and `MileageNoteService` uses the hidden marker or the canonical default `"Mileage reimbursement: ... x ... = ..."` line as idempotency proof. Public signature text alone is not trusted. Custom templates that do not render the canonical default line should include `{{marker}}` or the service appends the marker automatically. Team/Conversions admin views and CSVs take an optional `userId` filter backed by `GET /api/mileage/options/users`. Diagnostics returns setup checklist items and webhook operational health; stale pending jobs and failed jobs surface as warnings. The Conversions table renders `SKIPPED` rows with plain-language skip reason labels. Admin Insights aggregates conversion health without adding Prometheus tags.

UI/UX uplift notes (2026-06-06+): the printable report renders the **Amount/Total columns as money at 2 dp** (`HALF_UP`) — `MileageReportRendererTest`/`MileageReportControllerTest` lock `25.39`/`18.00`, not full precision; don't "fix" it back. The create-form **Project picker is a searchable `<input list="project-options">` + `<datalist>`** (not a `<select>`); `resolveProjectId` maps the typed name → `projectId`. The iframe shell is a CSP-safe ARIA `role="tablist"` (roving tabindex; `MileageSecurityTest` locks the panel/tab markup), error toasts are `role="alert"` + dismissible (no auto-vanish), Mine/Team/Conversions tables paginate (`settings-tables.js renderPager`, `&page=` after the locked `pageSize=50" + query` strings) + collapse to mobile cards via `data-label`, and admin nav includes Insights. Dark-mode primary buttons use `--on-accent #07232a` ink (≈6.3:1) — never white-on-teal (2.42:1 WCAG fail). All styling stays in `settings.css`/`report.css`/`packet.css`; do not introduce inline `<style>`/`style=`/`onclick=` (tests assert their absence). Static-asset deploy probes include every `settings*.js` file plus packet assets.
...
- The stuck-job reaper should reopen paired webhook events stuck in `PROCESSING` when it resets stale CLAIMED jobs back to PENDING; do not leave the event dedupe guard permanently closed after a worker crash.
...
The project is in maintenance mode. There is no debt list. There are no scale gaps. There are no open PRs. Production runs `9d89508d` (or later); prove worker health from `tasks_scheduled_execution_seconds_count{code_function="pollAndProcess",outcome="SUCCESS",exception="none"}` growing at the configured `MILEAGE_WORKER_POLL_DELAY_MS`-derived rate with zero exception tags. Suggest concrete maintenance items only if asked, and frame them as optional.
