...
- **Add-on-form creates do NOT carry the `(Clockify category charge: X)` annotation — only native conversions do.** A synchronous reconcile (a second `updateFlatExpense` to the just-created expense) was tried twice (PR #4/#5) and reverted after live QA 2026-06-05: that second write races Clockify's own `EXPENSE_CREATED` webhook and proved unreliable in production (the charge computed correctly — `categoryCharge=9.05`, `noteChanged=true` — but the write hung/never persisted, and the create stalled ~20s on `saveAndFlush`). So when you smoke an **add-on-create**, do NOT expect an extra `EXPENSE_UPDATED`/`SKIPPED` and do NOT assert the parenthetical on the created note — the note is just `Mileage reimbursement: <miles> miles x <rate> = <amount>. Created/converted by Mileage for Clockify.`. The divergence is bounded to integer-cent rounding by Fix 1A's category-price sync. A `+1 updateFlatExpense` on an add-on create is expected ONLY in the webhook-reserved-first race (re-marking the persisted conversion id), not for the charge.
