...
| New Flyway migrations must be numbered AFTER the highest migration already present in the repo and after the highest applied production migration. Production applied product migrations through `V24__add_mileage_trip_evidence.sql` on the 2026-06-16 OCI deploy at git `a540d30`; the next new migration must be V25 or higher. | Flyway validates strict ordering and crashes boot with `Detected resolved migration not applied to database: N`. The d11e2088 deploy crashed on this with V7. |
