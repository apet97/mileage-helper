...
- An **add-on-form create** can now also fire one extra `EXPENSE_UPDATED` (loop-guard `SKIPPED`) of its own: after the create, `MileageApiController` reissues a single `updateFlatExpense` to stamp the `(Clockify category charge: X)` annotation when the real Clockify charge differs from the rate amount. The charge is resolved by `resolveClockifyCategoryCharge`, which prefers the create-response `total` but **falls back to a `gateway.getExpense` snapshot when the create response omits `total` — and the addon-API expense-create response DOES omit it** (live-QA 2026-06-05: without the snapshot fallback the add-on-create note shipped with no annotation while native conversions annotated correctly). So during an add-on-create smoke, expect up to `+1` worker timer / `+1 SKIPPED` beyond the create itself, AND assert the created expense note actually carries `(Clockify category charge: …)` whenever the category's integer-cent price diverges from the rate (e.g. rate `0.725` → category `0.73/mile` → a 12.4-mile create charges `9.05` vs recorded `8.99`). If the charge matches the rate amount, no extra update fires. Do NOT trust the create-response `total` alone — that is the exact production-only assumption that silently dropped the annotation.
