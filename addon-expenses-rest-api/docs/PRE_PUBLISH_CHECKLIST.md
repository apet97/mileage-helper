...
- 2026-05-28 ship + redeploy at git `3fbe57c` (`feat(mileage): enrich CSV exports with project_name and pin multipart limits`): `./scripts/verify-publish.sh` passed end-to-end including Colima-backed reactor, `node --check` on both settings JS assets, `node scripts/test-mileage-date-helpers.mjs`, `git diff --check`, `gitleaks` (no leaks), and Docker image build.
