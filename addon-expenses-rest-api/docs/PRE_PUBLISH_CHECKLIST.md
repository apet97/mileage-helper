...
- 2026-06-16 OCI deploy at git `a540d30`: `./scripts/verify-publish.sh` passed end-to-end, local `mvn -pl addon-expenses-rest-api -am package -DskipTests` built jar SHA `72494cb1f1c31aadff80ba2d12a95525cf28364bafd20e012451657d5bddf270`, remote `/opt/mileage-for-clockify/mileage-for-clockify.jar` matched the same SHA after backup `/opt/mileage-for-clockify/mileage-for-clockify.jar.20260616T200100Z.bak`, and `mileage-for-clockify.service` restarted active at `Tue 2026-06-16 20:01:10 GMT`. Flyway applied V22-V24 and production is now at product migration V24. Hosted probes passed for health (`200 UP`), manifest (`200`, key `mileage-for-clockify`, schema `1.5`), every settings asset, report assets, packet assets, icon, unauthenticated iframe/report/reimbursement-packet guards (`401`), metric families (`mileage_conversion_outcome_total` 8 lines, `mileage_webhook_queue_depth` 1 line, `mileage_webhook_job_process_seconds_count` 1 line), scheduler liveness (`pollAndProcess` 75 -> 87, `reapStuckJobs` 2, `reconcilePendingNotes` 2), and no PII metric tags. Fresh `journalctl -u mileage-for-clockify.service --since "2026-06-16 20:01:00 UTC"` error scan was empty. Live Clockify smoke was skipped.
