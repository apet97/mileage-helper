...
- `addon-expenses-rest-api`: Mileage add-on application, UI, manifest, settings, webhooks, conversions, async webhook worker (`worker/` package), Prometheus metrics (`metrics/` package), printable per-user mileage report (`report/` package — `MileageReportRenderer` + `/iframe/report` controller), Dockerfile, compose file (two services — `addon` web pod and `addon-worker`), product migrations through `V19__trim_mileage_conversion_audit_surface.sql`, and add-on docs.
...
- Fresh workspaces with no saved settings row default the rate to `0.725` (seeded in `MileageSettingsService.defaults()`); existing rows and the Clockify-category `unitPrice` adoption path are unchanged.
...
- `GET /iframe/report` is a server-rendered printable per-user reimbursement report (no PDF library; browser print-to-PDF). It is an `/iframe/**` route so it authenticates via the `auth_token` query param and uses external `/assets/mileage/report.css` and `/assets/mileage/report.js` (CSP forbids inline). Admin may pass `userId`; a non-admin always gets their own (foreign `userId` is forced back to the requester). Capped at 1000 rows with a visible truncation notice; totals reflect only the shown rows.
- Webhooks: `EXPENSE_CREATED`, `EXPENSE_UPDATED`, `EXPENSE_DELETED`, `EXPENSE_RESTORED`.
- DB tables: `mileage_workspace_settings`, `mileage_conversion`. `V19__trim_mileage_conversion_audit_surface.sql` removes obsolete `currency`, `raw_event_hash`, and `clockify_request_id` audit columns and removes the unused `FETCHED` status. Platform tables (in `addon-db`): `addon_installations`, `addon_webhook_tokens`, `addon_workspace_settings`, `addon_webhook_events`, and `addon_webhook_jobs` (G1 async queue, Flyway V17; lifecycle `PENDING → CLAIMED → COMPLETED|FAILED`). Migrations now continue from the highest applied production version to avoid Flyway out-of-order validation failures.
...
- Generated Clockify notes preserve any user-typed note (prepended above the canonical line, separated by a blank line) and are otherwise exact, e.g. `Mileage reimbursement: 1 mile x 0.725 = 0.725. Created/converted by Mileage for Clockify.` For native conversions, the canonical line additionally explains the actual Clockify category charge whenever it differs from the addon's calculated amount — Clockify computes the unit-priced category total from the integer-cent category price, which can differ from the addon's higher-precision rate — e.g. `Mileage reimbursement: 12.4 miles x 7.25123 = 89.915252 (Clockify category charge: 89.90). Created/converted by Mileage for Clockify.` The `settings.rate()` keeps full precision (NOT rounded) and the addon's own recorded amounts (Mine/Team/Conversions) are unchanged — only the Clockify note reconciles the two. The charge comes from `ClockifyExpenseSnapshot.total` (cents) via `MileageConversionService.clockifyCategoryCharge`; addon-created expenses pass `null`. Note idempotency: a note already containing `MileageNoteService.MARKER_PREFIX` or the `Created/converted by Mileage for Clockify.` signature is returned unchanged (no re-stacking on retry/restore).
