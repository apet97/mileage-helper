...
- The create-form Project picker is a searchable combobox (`<input id="field-project" list="project-options">` + `<datalist>`), not a `<select>`. `settings.js loadProjects` sorts options alphabetically and builds a name→id map; `resolveProjectId` maps the typed name back to `projectId` for `mileagePayload` (unknown text → no project). Scopes and the create contract are unchanged.
