...
- **Add-on-created expenses deliberately do NOT carry the `(Clockify category charge: X)` annotation.** PR #4/#5 attempts were reverted after live QA 2026-06-05: stamping it needs a SECOND synchronous `updateFlatExpense` to the just-created expense, which races Clockify's own `EXPENSE_CREATED` webhook; prod logs showed the charge computed correctly but the second write hung/never persisted (plus a ~20s `saveAndFlush` stall from same-row contention). `MileageApiController.createExpense` writes only the create note and fires `updateFlatExpense` ONLY for the webhook-reserved-first race (to re-mark the persisted conversion id). Fix 1A's category-price sync keeps the divergence to integer-cent rounding; records stay exact. Do NOT re-add a synchronous add-on-create charge reconcile. Note idempotency: a note already containing `MileageNoteService.MARKER_PREFIX` or the `Created/converted by Mileage for Clockify.` signature is returned unchanged (no re-stacking on retry/restore).
