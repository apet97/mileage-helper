...
- Current dev/live-test mode while Railway is unavailable: `scripts/dev-tunnel.sh --build` via Cloudflared quick tunnel. Quick-tunnel URLs are ephemeral; paste the printed `/manifest` URL into Clockify after every restart and do not treat a previous `trycloudflare.com` hostname as current truth.
