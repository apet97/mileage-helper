...
21. New `addon-db` platform migrations must be numbered AFTER the highest applied mileage migration in production (currently V17 is the first slot, V16 is the highest applied). Do not insert migrations into gaps below V16 — Flyway validates strict ordering and crashes boot with `Detected resolved migration not applied to database: N`. The 2026-05-30 deploy `d11e2088` crashed on exactly this when V7 was used; renumber to ≥ V17.
