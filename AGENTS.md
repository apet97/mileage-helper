...
- `GET /iframe/report` is a server-rendered printable per-user reimbursement report (no PDF library; browser print-to-PDF). It is an `/iframe/**` route so it authenticates via the `auth_token` query param and uses external `/assets/mileage/report.css` and `/assets/mileage/report.js` (CSP forbids inline). Admin may pass `userId`; a non-admin always gets their own (foreign `userId` is forced back to the requester). The report includes only `CONVERTED` rows so the printed Total reflects actual Clockify reimbursements (`DRY_RUN`/`FAILED`/`CONVERTING`/`SKIPPED` are excluded). Capped at 1000 rows with a visible truncation notice; totals reflect only the shown rows.
