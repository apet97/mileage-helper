...
21. New Flyway migrations must be numbered AFTER the highest migration already present in the repo and after the highest applied production migration. This repo now contains pending product migrations through `V24__add_mileage_trip_evidence.sql`; hosted evidence still proves production only through V20 until a deploy applies V21-V24. Do not insert migrations into gaps below either maximum — Flyway validates strict ordering and crashes boot with `Detected resolved migration not applied to database: N`. The 2026-05-30 deploy `d11e2088` crashed on exactly this when V7 was used; the next new migration after this branch must be V25 or higher.
