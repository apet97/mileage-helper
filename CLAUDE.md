...
- **Add-on-created expenses deliberately do NOT carry the `(Clockify category charge: X)` annotation.** Two earlier attempts (PR #4 reading the create-response `total`; PR #5 falling back to a `getExpense` snapshot) were reverted after live QA on 2026-06-05: stamping the charge requires a SECOND synchronous `updateFlatExpense` to the just-created expense on the request thread, which races Clockify's own `EXPENSE_CREATED` webhook for the same expense. Production logs showed the logic computed the right charge (`categoryCharge=9.05`, `noteChanged=true`) but the second write hung/never persisted (the create path also stalled ~20s on `saveAndFlush` from the same-row contention), so the annotation never landed. `MileageApiController.createExpense` therefore writes only the create note and fires `updateFlatExpense` ONLY for the webhook-reserved-first race (to re-mark the note with the persisted conversion id), never to stamp the charge. Fix 1A (settings-save category-price sync) keeps the divergence to integer-cent rounding (e.g. recorded 8.99 vs charged 9.05 on a 12.4-mi expense), and the add-on's own records stay exact. Do NOT re-add a synchronous category-charge reconcile on the add-on-create path — it is unreliable; only a deferred/out-of-band reconcile could do it safely. Note idempotency: a note already containing `MileageNoteService.MARKER_PREFIX` or the `Created/converted by Mileage for Clockify.` signature is returned unchanged (no re-stacking on retry/restore).
