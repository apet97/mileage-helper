...
- Fresh workspaces with no saved settings row default the rate to `0.725` (seeded in `MileageSettingsService.defaults()`); existing rows and the Clockify-category `unitPrice` adoption path are unchanged. Only the in-memory default for a row-less workspace changes — null-rate rows still take the Clockify `unitPrice` adoption path.
...
- `GET /iframe/report` is a server-rendered printable per-user reimbursement report (no PDF library; browser print-to-PDF). Admin may pass `userId`; a non-admin always gets their own (a foreign `userId` from a non-admin is ignored and forced to the requester). Uses external `/assets/mileage/report.css` and `/assets/mileage/report.js` (CSP forbids inline). It is an `/iframe/**` route so it authenticates via the `auth_token` query param; `report.js` strips the token from the URL after load. `from`/`to` are required. Capped at 1000 rows per report; a truncation notice is shown (and totals reflect only the shown rows) when exceeded.
