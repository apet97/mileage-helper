...
- Saving settings (`PUT /api/mileage/settings`) best-effort syncs the Clockify Mileage category's unit price to the saved rate. After persisting, `MileageSettingsController.syncMileageCategoryPrice` reads effective settings and, when a rate and a `mileageCategoryId` are present, calls `gateway.createOrRepairMileageCategory(workspaceId, rate)` so a unit category (which forces `total = miles × priceInCents`) charges the intended amount. It is best-effort: a Clockify outage is caught (`InterruptedException` re-interrupts; `IOException`/`RuntimeException` → `log.warn`) and never fails the save. This is the primary defense against the rate↔category-price divergence; the note annotation above documents the residual integer-cent rounding gap.
...
- Generated Clockify notes preserve any user-typed note (prepended above the canonical line, separated by a blank line) and are otherwise exact, e.g. `Mileage reimbursement: 1 mile x 0.725 = 0.725. Created/converted by Mileage for Clockify.` For native conversions AND add-on-created expenses, the canonical line additionally explains the actual Clockify category charge whenever it differs from the addon's calculated amount — Clockify computes the unit-priced category total from the integer-cent category price, which can differ from the addon's higher-precision rate — e.g. `Mileage reimbursement: 12.4 miles x 7.25123 = 89.915252 (Clockify category charge: 89.90). Created/converted by Mileage for Clockify.` The `settings.rate()` keeps full precision (NOT rounded) and the addon's own recorded amounts (Mine/Team/Conversions) are unchanged — only the Clockify note reconciles the two. For native conversions the charge value comes from `ClockifyExpenseSnapshot.total` (cents) via `MileageConversionService.clockifyCategoryCharge`. For add-on-created expenses it comes from the `total` field of the Clockify create response via `MileageApiController.clockifyCategoryCharge`: after the create, when the response `total` differs from the rate amount the controller reissues ONE `gateway.updateFlatExpense` to stamp the same `(Clockify category charge: X)` annotation (the resulting `EXPENSE_UPDATED` webhook is absorbed by the loop guard); when `total` matches or is absent, no second update fires. Note idempotency: a note already containing `MileageNoteService.MARKER_PREFIX` or the `Created/converted by Mileage for Clockify.` signature is returned unchanged (no re-stacking on retry/restore).
...
- New Flyway migrations must be numbered AFTER the highest applied migration in production (currently V19 after the maintenance cleanup). Do not insert migrations into gaps below the production maximum — Flyway validates strict ordering and will crash boot with `Detected resolved migration not applied to database: N`. The 2026-05-30 deploy `d11e2088` crashed on this exact failure when V7 was used; continue from the current maximum instead.
