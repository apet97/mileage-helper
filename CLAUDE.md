...
- New `addon-db` platform migrations must be numbered AFTER the highest applied mileage migration in production (currently V17 lands after V16). Do not insert migrations into gaps below V16 — Flyway validates strict ordering and will crash boot with `Detected resolved migration not applied to database: N`. The 2026-05-30 deploy `d11e2088` crashed on this exact failure when V7 was used; the renumber to V17 (commit `33190cd`) is the fix.
