...
- `com.cake.clockify.addon.mileage.report`: printable per-user mileage report — pure `MileageReportRenderer` (CSP-safe HTML) and the `/iframe/report` controller (per-user, admin may pass `userId`).
...
- User APIs: `GET /api/mileage/create-context`, `GET /api/mileage/mine`, `GET /api/mileage/mine.csv`, `POST /api/mileage/preview`, `POST /api/mileage/expenses`, `GET /iframe/report` (printable per-user report; non-admin always gets their own).
...
- Generated Clockify notes preserve any user-typed note (prepended above the canonical line, separated by a blank line) and are otherwise exact, e.g. `Mileage reimbursement: 1 mile x 0.725 = 0.725. Created/converted by Mileage for Clockify.` For native conversions, the canonical line additionally explains the actual Clockify category charge whenever it differs from the addon's calculated amount — Clockify computes the unit-priced category total from the integer-cent category price, which can differ from the addon's higher-precision rate — e.g. `Mileage reimbursement: 12.4 miles x 7.25123 = 89.915252 (Clockify category charge: 89.90). Created/converted by Mileage for Clockify.` The `settings.rate()` keeps full precision (NOT rounded) and the addon's own recorded amounts (Mine/Team/Conversions) are unchanged — only the Clockify note reconciles the two. The charge value comes from `ClockifyExpenseSnapshot.total` (cents) via `MileageConversionService.clockifyCategoryCharge`; addon-created expenses pass `null` (no annotation, total unknown at create time). Note idempotency: a note already containing `MileageNoteService.MARKER_PREFIX` or the `Created/converted by Mileage for Clockify.` signature is returned unchanged (no re-stacking on retry/restore).
...
- `GET /iframe/report` is a server-rendered printable per-user reimbursement report (no PDF library; browser print-to-PDF). Admin may pass `userId`; a non-admin always gets their own (a foreign `userId` from a non-admin is ignored and forced to the requester). Uses external `/assets/mileage/report.css` and `/assets/mileage/report.js` (CSP forbids inline). It is an `/iframe/**` route so it authenticates via the `auth_token` query param; `report.js` strips the token from the URL after load. `from`/`to` are required. The report includes ONLY `CONVERTED` rows (queried via `findAllByWorkspaceIdAndUserIdAndStatusAndExpenseDateBetween(..., CONVERTED, ...)`) so the printed Total reflects money actually written to Clockify — `DRY_RUN`/`FAILED`/`CONVERTING`/`SKIPPED` rows retain a `calculatedAmount` but are excluded because the report has no Status column. Capped at 1000 rows per report; a truncation notice is shown (and totals reflect only the shown rows) when exceeded.
